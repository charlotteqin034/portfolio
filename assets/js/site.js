...
window.PROJECTS = [
  {
    ...
    body: [
      'I had the opportunity to work with Sony on a creative project focused on transmedia. We wanted to flip traditional gameplay on its head: instead of relying on intentional input, the standard for most games, we used involuntary physiological signals to influence the world around the player. Our finished product was a custom-modeled VR environment with music tied to the user\'s biofeedback, where different brainwaves controlled elements like notes, rhythm, and key, heart rate drove the tempo, and dynamic visuals shifted alongside the music. My role centered on building the EEG-to-music translation system, learning and applying Ableton\'s Max for Live coding language to map brainwave readings to musical elements. We presented the project internally to Sony employees in the US and Japan, and I spoke about the work on a panel at South by Southwest 2026.'
    ],
    highlights: [
  ...
  },
  {
    ...
    body: [
      'During my time working for Prox, a grocery deals startup, the existing search relied purely on keyword matching and ignored the intent behind each query. I worked on cleaning data (categorizing, brand-matching, and normalizing size information for retail grocery products) and built a serverless data enrichment pipeline in TypeScript on Supabase Edge Functions. I grouped products into common search patterns, paired with LLM fallback for uncertain categorizations, which added semantic functionality, matching by meaning rather than just keywords. Since this work was for a startup, the available tech was limited, so I worked to minimize and continually reduce AI API usage to control costs. The new system reduced irrelevant results by over 70%.'
    ],
    highlights: [
  ...
  },
  {
    ...
    body: [
      'In partnership with Microsoft for Startups, this 24-hour hackathon gave each team access to Microsoft Azure credits. My team and I wanted to leverage as many Azure services as possible (Redis, Web PubSub, OpenAI, Speech, Container Apps, and Static Web Apps) while creating a unique software product. We developed Agents on Air, a real-time AI podcast platform that generates conversational episodes between AI hosts: an LLM generates a general outline, then each host\'s script is generated in real time, responding to the previous message while following the outline. This lets listeners ask questions and add to the conversation through voice chat or text input, with low latency. My time was spent designing and developing the entire backend and Azure service orchestration, making sure script generation, conversation flow, and user interaction happened without significant pauses.'
    ],
    highlights: [
  ...
  },
  {
    ...
    body: [
      'This 24-hour makeathon\'s theme was "Tech for Good," encouraging us to build physical products rather than software. Coming from a traditional software background, this was one of my first experiences building with hardware. Our product was an electric composter shaped like a decorative plant stake: a user drops in fruit peels which are dehydrated and composted, then automatically fed straight into the plant pot. My responsibilities centered on wiring the Arduino to the temperature sensor, heater modules, LCD, and servos, then rigging everything into the final build, which proved harder than anticipated due to space constraints. I was building until the very last moment, but we produced a working proof of concept, pitched it to a panel of judges, and won the environmental track.'
    ],
    highlights: [
  ...
  },
  {
    ...
    body: [
      'After countless problem discovery calls with event planners, lighting designers, and production managers, my cofounder and I decided to work within the live entertainment industry to address the challenge of planning and designing an event. The most important feature was our ability to leverage drone photogrammetry to generate 3D models of an event venue, saving the time and cost of sourcing 3D modelers to build one by hand: the drone captures hundreds of photos of the venue in flight, OpenDroneMap processes them into the model, and the model is uploaded into Nethra, where planners can design the event through an intuitive interface. My work centered on UI/UX design of the 3D editor and developing the technical pipeline from drone photogrammetry to software. We pitched the product to a panel of investors and set up design partnerships with Tomorrowland Brasil, Afterlife, and Lollapalooza Brasil.'
    ],
    highlights: [
